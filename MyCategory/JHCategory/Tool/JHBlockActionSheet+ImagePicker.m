...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if (imagePicker.photoBlock) {
        ...
        if (image.size.width > 512) {
            ...
            CGFloat scaleSize = image.size.height / image.size.width;
            
            image = [imagePicker imageWithImage:image scaledToSize:CGSizeMake(512, 512 * scaleSize)];
                         ...
                         pickerCompletion:(JHPhotoPicker_t)photoPicker
{
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        return [[JHBlockActionSheet alloc] initActionSheetWithTitle:title cancelButtonItem:cancelItem destructiveButtonItem:nil otherButtonItems:cameraItem, libraryItem,nil];
